			...
			>
				<nav className="fixed top-0 left-0 right-0 z-50 pt-2 pb-2 mx-auto bg-black sm:py-2 sm:static sm:rounded-tr-md sm:rounded-tl-md">
					...
					<ul className="items-center m-auto sm:h-8 w-app_inner_width sm:flex">
						<UserProfile isOpenDrawer={isOpenDrawer} onOpenDrawer={handleOpenDrawer} auth={auth} />
						...
						{auth.user && (
							<>
								...
								<NavLinkIcon href="/favorite" icon={<PiHeartStraightLight />}>
									Favorite
								</NavLinkIcon>
								{/* <NavLinkIcon
                                    href="/community"
						...
						{auth.user ? (
							<NavLinkIcon href="/logout" isSelfToRight={true} icon={<PiSignOutLight />}>
								Logout
							</NavLinkIcon>
						) : (
							<div className="flex ms-auto">
								...
								<NavLinkIcon href="/register" icon={<PiNotePencilLight />}>
									Register
								</NavLinkIcon>
								<NavLinkIcon href="/login" icon={<PiDoorOpenLight />}>
									Login
								</NavLinkIcon>
							</div>
						)}
	...
	return (
		<div className="hidden sm:block bg-gradient-to-r from-[#C991E9] to-[#F4BEA7] sm:h-10 lg:h-12  mx-auto ">
			<ul className="flex justify-center gap-32 px-5 sm:pt-1 lg:pt-2">
				...
				<li>
					<NavLink active={false} href="#" onClick={() => toast('Sorry manga page not yet available 😶‍🌫️')}>
						Manga
					</NavLink>
				</li>
				<li>
					<NavLink active={false} href="#" onClick={() => toast('Sorry drama page not yet available 😶‍🌫️')}>
						Drama
					</NavLink>
				</li>
				<li>
					<NavLink active={false} href="#" onClick={() => toast('Sorry news page not yet available 😶‍🌫️')}>
						News
					</NavLink>
				</li>
			</ul>
	...
	return (
		<>
			...
			<div className="flex items-center pt-2 bg-black justify-evenly sm:hidden">
				{auth.user ? (
					<>
						<Link className="flex flex-col items-center" href="/history">
							<PiTelevisionLight />
							<span className="mx-auto text-xs text-stone-50">History</span>
						</Link>
						<Link className="flex flex-col items-center" href="/favorite">
							<PiHeartStraightLight />
							<span className="mx-auto text-xs text-stone-50">Favorite</span>
						</Link>
						<Link className="flex flex-col items-center" href="/community">
							<PiPersonArmsSpreadThin />
							<span className="mx-auto text-xs text-stone-50">Community</span>
						</Link>
						<Link className="flex flex-col items-center" href="/logout">
							<PiDoorOpenLight />
							<span className="mx-auto text-xs text-stone-50">Logout</span>
						</Link>
					</>
				) : (
					<>
						...
						<Link className="flex flex-col items-center" href="/favorite">
							<PiNotePencilLight />
							<span className="mx-auto text-xs text-stone-50">Register</span>
						</Link>
						<Link className="flex flex-col items-center" href="/login">
							<PiDoorOpenLight />
							<span className="mx-auto text-xs text-stone-50">Login</span>
						</Link>
					</>
				)}
			</div>
	...
	return (
		<li className="flex justify-evenly">
			...
			<Link href="/" className="mx-auto text-center sm:text-left sm:mx-2">
				<p className="text-[#F4BEA7] text-xs  tracking-wide">{auth.user ? 'Premium Member' : 'Global User'}</p>
				<p className="text-xs max-w-[250px] truncate text-center text-white sm:text-left">
					{auth.user ? auth.user.name : 'Hi ! Welcome to Apple Juice.'}
				</p>
			</Link>
			{/* render hamburger button when sm: */}
			<button className="relative self-start group sm:ms-auto sm:hidden" onClick={e => onOpenDrawer(e)}>
				<div
					...
					>
						...
						<div
							className={`absolute items-center justify-between transform transition-all duration-500 top-2.5  ${
								isOpenDrawer ? 'translate-x-0' : '-translate-x-10'
...
function NavLinkIcon({ children, href, isSelfToRight = false, icon }) {
	return (
		<li className={`hidden sm:block self-start px-4 ${isSelfToRight ? 'ms-auto' : ''}`}>
			<Link href={href}>
